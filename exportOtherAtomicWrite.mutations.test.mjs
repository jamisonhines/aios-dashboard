...
try {
  ...
  // Differential proof: the later pre-rename owner check can reject old A after the
  ...
  const mutationCases=[
    ...
    // I4 clears the pre-work guard before B starts. Mutate only this executable
    // downstream fence so a failure proves its own fixture and assertion.
    ["pre-rename owner check", "    if (beforeRename && !(await beforeRename())) throw lostLockError();\n", "I4 stale A must fail specifically at pre-rename fence", (source) => source.replace("    if (beforeRename && !(await beforeRename())) throw lostLockError();\n", "")],
    // A bad release deletes B's lock, which otherwise makes B's later pre-rename guard fail
    // in I1 and I3 before C can observe the release defect. Disable those masking checks with it.
    ["release owner check", "    if (token && await isOwner()) await fs.rm(lockPath, { recursive: true, force: true }).catch(() => {});", "I3 release guard: C must specifically stay busy while replacement B owns the lock", (source) => source.replace("    if (beforeRename && !(await beforeRename())) throw lostLockError();", "    if (beforeRename && !filePath.endsWith(\"stolen.json\") && !filePath.endsWith(\"release.json\") && !(await beforeRename())) throw lostLockError();").replace("    if (token && await isOwner()) await fs.rm(lockPath, { recursive: true, force: true }).catch(() => {});", "    if (token) await fs.rm(lockPath, { recursive: true, force: true }).catch(() => {});")],
    // I2 mutates only stale steal-coordination recovery; the named recover.json fixture
    // must stay busy without it, rather than passing through a different guard.
