...
export async function main(vaultRoot = process.argv[2] || process.cwd()) { const snapshot = await buildAgentModelsSnapshot({ vaultRoot }); const output = path.join(vaultRoot, "Operations", "agent-models.json"); const result = await withOwnedExportLock(output, () => writeJsonAtomic(output, snapshot)); if (result.busy) { console.log(`agent-models export busy; another writer holds ${output}.lock`); return; } console.log(`agent-models: ${snapshot.agents.length} agent(s) -> ${output}`); }
