...
export async function main(vaultRoot = process.argv[2] || process.cwd()) { const snapshot = await buildAgentModelsSnapshot({ vaultRoot }); const output = path.join(vaultRoot, "Operations", "agent-models.json"); await fs.mkdir(path.dirname(output), { recursive: true }); await fs.writeFile(output, JSON.stringify(snapshot, null, 2) + "\n", "utf8"); console.log(`agent-models: ${snapshot.agents.length} agent(s) -> ${output}`); }
