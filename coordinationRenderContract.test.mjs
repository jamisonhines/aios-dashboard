...
function renderLedger(mod, content) {
  const [view] = computeCoordinationView([{ slug: "p", ledgerContent: content, questionsContent: null }], NOW);
  const pills = El();
  ...
  mod.__renderCoordinationBody(
    ...
    body,
    view,
    { expanded: new Set(), coordinationQuestionFilter: new Map() },
    () => {},
    {}
  );
...
// ---------------------------------------------------------------------------
{
  ...
  assert.ok(emptyView.warnings.includes(MISLEADING_LINE));
